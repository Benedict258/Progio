...
export default function MyPrivateGrantsPage() {
  ...
  const refresh = async () => {
    try {
      setLoading(true);
      const data = await fetchPrivateOpportunities("grant");
      setItems(data);
    } catch {
      setError("Failed to load private grants");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleImport = async () => {
    ...
    try {
      setImporting(true);
      setError("");
      const parsed = await parseExternalUrl(importUrl.trim(), "grant");
      if (parsed) {
  ...
  const handleStartApplication = async (opp: PrivateOpportunity) => {
    try {
      setSubmitting(true);
      const app = await createApplicationFromOpportunity(opp.id, "grant");
      if (app) {
        router.push(`/grants/applications/${app.id}`);
      } else {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const tags = fieldTags
        ...
        .map((t) => t.trim())
        .filter(Boolean);
      let eligibilityObj: Record<string, unknown> | null = null;
      if (eligibility.trim()) {
        try {
      ...
      <div className="p-8 max-w-4xl">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          <div className="flex items-start justify-between mb-4">
            <div>
              <span className="inline-block px-2.5 py-0.5 text-xs font-medium bg-indigo-50 text-indigo-700 rounded-full mb-2">
                Grant
              </span>
  ...
  if (view === "edit") {
    return (
      <div className="p-8 max-w-4xl">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          <h2 className="text-lg font-semibold text-slate-900 mb-4">Edit Grant Opportunity</h2>
          {error && <p className="text-sm text-red-600 mb-4">{error}</p>}
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">Title *</label>
              <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Innovation Research Fund" className="w-full px-3 py-2.5 bg-white border border-slate-200 rounded-lg text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">Provider *</label>
              <input type="text" value={provider} onChange={(e) => setProvider(e.target.value)} placeholder="e.g. National Science Foundation" className="w-full px-3 py-2.5 bg-white border border-slate-200 rounded-lg text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="text" value={awardRange} onChange={(e) => setAwardRange(e.target.value)} placeholder="e.g. $10,000 - $50,000" className="w-full px-3 py-2.5 bg-white border border-slate-200 rounded-lg text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="text" value={fieldTags} onChange={(e) => setFieldTags(e.target.value)} placeholder="Comma separated: STEM, Engineering, AI" className="w-full px-3 py-2.5 bg-white border border-slate-200 rounded-lg text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
            </div>
            <div className="sm:col-span-2">
              <label className="block text-sm font-medium text-slate-700 mb-1.5">Description</label>
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Brief description of the opportunity..." rows={3} className="w-full px-3 py-2.5 bg-white border border-slate-200 rounded-lg text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none" />
            </div>
            <div className="sm:col-span-2">
              <label className="block text-sm font-medium text-slate-700 mb-1.5">Eligibility Criteria</label>
              <textarea value={eligibility} onChange={(e) => setEligibility(e.target.value)} placeholder="e.g. Graduate students, GPA 3.0+, STEM fields" rows={2} className="w-full px-3 py-2.5 bg-white border border-slate-200 rounded-lg text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none" />
            </div>
            <div className="sm:col-span-2">
  ...
  return (
    <div className="p-8 max-w-4xl">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 mb-2">My Private Grants</h1>
          <p className="text-slate-600">Add grant opportunities you found outside the platform.</p>
        </div>
        <button
            ...
            {activeTab === "import" ? (
              <div>
                <p className="text-sm text-slate-600 mb-4">
                  Paste a URL to an opportunity page. We&apos;ll extract the key details automatically.
                </p>
                <div className="flex gap-3">
                  <input
                    ...
                    placeholder="https://nsf.gov/funding/opportunity/..."
                ...
                <p className="text-xs text-slate-400 mt-2">
                  Works best with grant listing pages. PDF import coming soon.
                </p>
              </div>
            ) : (
              <div>
                <h2 className="text-lg font-semibold text-slate-900 mb-4">
                  {selected ? "Edit Grant Opportunity" : "Add Private Grant"}
                </h2>
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1.5">Title *</label>
                  ...
                  <div className="sm:col-span-2">
                    <label className="block text-sm font-medium text-slate-700 mb-1.5">Eligibility Criteria</label>
                    <textarea value={eligibility} onChange={(e) => setEligibility(e.target.value)} placeholder="Graduate students, GPA 3.0+, STEM fields..." rows={2} className="w-full px-3 py-2.5 bg-white border border-slate-200 rounded-lg text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none" />
                  </div>
                  <div className="sm:col-span-2">
                    <label className="block text-sm font-medium text-slate-700 mb-1.5">Source URL</label>
                    <input type="url" value={sourceUrl} onChange={(e) => setSourceUrl(e.target.value)} placeholder="https://..." className="w-full px-3 py-2.5 bg-white border border-slate-200 rounded-lg text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
      ...
      <div className="space-y-3">
        {loading ? (
          <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
            ...
            <p className="text-slate-500">Loading private grants...</p>
          </div>
        ) : items.length === 0 ? (
          <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-slate-900 mb-2">No private grants</h3>
            <p className="text-slate-500 max-w-md mx-auto">
              Add grant opportunities you found outside the platform, or import them from a URL.
            </p>
          </div>
        ) : (
          items.map((item) => (
